...
- **OAuth**: start OAuth/device flows for supported providers, poll status, and optionally submit callback `redirect_url`. iFlow remains visible as a runtime credential-file scope when the connected Core exposes those files; this panel does not advertise an unverified management import endpoint.
...
- Tagging a panel release such as `v1-lts-0.0.1` triggers `.github/workflows/release.yml` to publish `dist/management.html`.
- Use an annotated tag whose subject is the human summary and whose body contains exactly one `Companion-Core: v*-lts-*` line. The workflow uses that explicit contract plus GitHub `generate-notes`; it does not infer compatibility from release timestamps or PR order.
