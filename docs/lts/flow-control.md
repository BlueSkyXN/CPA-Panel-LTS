...
1. 在左侧导航「观测」分组打开「Flow 并发与频率」页面（`/flow-control`，按 Core 能力门控显示；旧的 `#/config?section=flow-control` 深链会重定向到该页）。执行、实时观察、资源采样三个服务端开关默认关闭；仅打开页面不创建配置。
...
4. 先使用只读预览，再通过 Flow 页面自带的差异确认和 `PUT /v0/management/config.yaml` 保存；保存只合并 flow-control 相关键，不回写其他可视化字段。预览不占位，也不证明路由或套餐可用。
5. 保存后手动刷新，核对草稿、文件期望配置、实际策略与失败提示。保存返回409不写文件；watcher 竞争或策略不能应用时保留最后有效策略，不全局阻止新请求，也不绕过旧限流。

Flow 页面“开始/停止实时”只控制当前页面订阅；离开 `/flow-control` 即取消订阅；后台标签暂停，断线只重连观察，服务端关闭实时后不自动改为轮询。关闭流控不取消已有模型执行；draining 仍计实际生产者名额。关闭期间的新调用不计入受控摘要，不能把它当全站并发。

模型、Key、账号选择保留未列出的既有引用。Flow 页面的可视化修改保留未管理 YAML 字段与注释；畸形已知字段显示错误，不把未知或非法规则替换为空规则。需要手工修复 YAML 结构时使用配置面板的源码编辑器，停用仍可保留原草稿。
