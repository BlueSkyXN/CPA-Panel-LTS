...
PR #85 的既有验证未覆盖两个组合场景：服务端同时重排/插入 payload，以及编辑包含跨规则 anchor/alias 的 payload。后续复查在 `53ea424` 上确认了错配未知字段、丢弃新增规则和静默丢弃草稿，不能将原有绿灯理解为这两个问题已修复。

本次 `codex/fix-payload-integrity` 按最小方案补齐：

- 加载时为五个 payload 配置块保存节点身份与解析值快照。应用修改前，只比较 dirty 配置块；服务端同时修改时抛出冲突，不再进入按位置配对。该前置门禁覆盖规则、filter、models、match/not-match 四类位置配对，也覆盖块外 anchor 引用值变化；未编辑配置块及无关字段仍可合并。
- 未编辑的参数对象和 alias 保留原节点；确需替换的 anchored 参数保留 anchor。仍被引用的 anchor 被删除、结构性 alias 无法安全展开或 YAML 转换失败时，明确失败，不返回原文冒充成功。
- 保存预览、确认保存和切换源码三个消费者均处理错误，保留草稿，不发出配置写请求；四语言提示区分冲突与转换失败，不输出底层 YAML 错误中的配置值。
- 原来的 6 项 payload 测试增加 19 项回归；首轮新增用例有 17 项失败，修复后专项 25 项全部通过。完整 `validate:lts` 为 340 项测试，contract、type-check、lint、build 通过。
- 配置专项浏览器覆盖保存前冲突、预览后冲突、跨规则 alias 正常保存、反复切换源码、引用损坏时不切页/不清草稿/零 PUT；原有布局、主题、语言与完整 mock smoke 也通过。

这是冲突检测与失败保护，不是自动三方合并。结构性 alias 不能安全编辑时明确阻止转换，不承诺自动展开；用户主动编辑源码的完整草稿保存语义保持不变。GitHub 精确 head、合并状态和最终 Core 验证以对应修复 PR 记录为准；无发布或部署。
