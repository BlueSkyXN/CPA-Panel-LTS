...
- 打 `v1-lts-0.0.1` 这类面板发布标签会触发 `.github/workflows/release.yml`，发布 `dist/management.html`。
- 使用 annotated tag：subject 写本版摘要，body 必须且只能包含一条 `Companion-Core: v*-lts-*`。工作流使用这份显式契约和 GitHub `generate-notes`，不再按发布时间或 PR 顺序推断兼容关系。
