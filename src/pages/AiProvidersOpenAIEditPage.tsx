...
export function AiProvidersOpenAIEditPage() {
  ...
  const hasTestableKeys = form.apiKeyEntries.some(
    (entry) => entry.apiKey?.trim() || entry.authIndex?.trim() || form.authIndex?.trim()
  );
  ...
  const connectivityConfigSignature = useMemo(() => {
    ...
    const modelsSignature = form.modelEntries
      ...
      .join('|');
    const entriesSignature = form.apiKeyEntries
      .map((entry) => `${entry.apiKey?.trim() ?? ''}:${entry.authIndex?.trim() ?? ''}`)
      .join('|');
    return [
      form.baseUrl.trim(),
      form.authIndex?.trim() ?? '',
  ...
  const runSingleKeyTest = useCallback(
    async (keyIndex: number): Promise<boolean> => {
      ...
      const entryKey = keyEntry?.apiKey?.trim() || '';
      const authIndex = keyEntry?.authIndex?.trim() || form.authIndex?.trim() || undefined;
      if (!entryKey && !authIndex) {
  ...
  const testAllKeys = useCallback(async () => {
    ...
    const validKeyIndexes = form.apiKeyEntries
      .map((entry, index) =>
        entry.apiKey?.trim() || entry.authIndex?.trim() || form.authIndex?.trim() ? index : -1
      )
